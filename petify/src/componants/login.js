...
import img from '../images/swal/1.gif'
...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (user.role === 'vet') {
        // Vets receive a token, so login the vet with token
        login(user, token);
        Swal.fire({
          title: 'Welcome Back, Vet!',
          ...
          localStorage.setItem('token', token); // Store token for vets
          navigate('/vet-dashboard'); // Redirect to the vet dashboard
        });
      } else {
        // For regular users, we handle the token and login process
        login(user, token);
        Swal.fire({
          title: 'Welcome Back to Petify!',
          text: 'You have successfully logged in.',
          imageUrl: img,
          imageWidth: 100,
          imageHeight: 100,
          ...
          localStorage.setItem('token', token); // Store token for regular users
